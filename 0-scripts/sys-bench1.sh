...
# This block ensures no 'fio' commands with 'rw=readwrite' or 'rw=write'
# are present in the script below the FIO_PROTECTION_TEST_END marker.
# This is a critical safety measure to prevent accidental data destruction.
SCRIPT_CONTENT=$(cat "$0")
FIO_PROTECTION_TEST_END_LINE=$(echo "$SCRIPT_CONTENT" | grep -n "^# FIO_PROTECTION_TEST_END" | cut -d: -f1)
...
if [ -n "$FIO_PROTECTION_TEST_END_LINE" ]; then
    ...
    if echo "$CONTENT_AFTER_MARKER" | grep -q -E "rw=(readwrite|write)"; then
        echo -e "\033[1m\033[31mERROR: Potentially destructive 'fio' write operation (rw=readwrite or rw=write) detected in this script below the FIO_PROTECTION_TEST_END marker. Exiting to prevent data loss.\033[0m" >&2
        exit 1
    fi
fi
# --- FIO_SAFETY_CHECK_END ---

# FIO_PROTECTION_TEST_END
